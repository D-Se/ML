...
```{r logo, echo=FALSE, out.height="10%", out.width="10%"}
...
2.  Install project tools and dependencies by running `devtools::install_github("D-Se/ML")` in the R console. This might require you to install `tensorflow` separately.
...
This project uses **R & C++** for core development, **Python** for neural nets, **HTML, CSS & JavaScript** for reporting.
...
├──── ... >>> regular R package functions. o sub-directory allowed [NO EDIT].
├── data/  >>> analysis pipeline data objects exported in ML package
